...
function addToCart(product) {
  // If product is just an ID, find it in the combined list
  if (typeof product === 'number' || typeof product === 'string') {
    var userProducts = JSON.parse(localStorage.getItem('seller_products') || '[]');
    var combined = SAMPLE_PRODUCTS.concat(userProducts);
    var found = combined.find(function(p) { return p.id == product; });
...
function loadFeaturedProducts() {
  ...
  if (!grid) return;
  var userProducts = JSON.parse(localStorage.getItem('seller_products') || '[]');
  var combinedProducts = SAMPLE_PRODUCTS.concat(userProducts);
  grid.innerHTML = combinedProducts.slice(0, 8).map(function(p) {
    var discount = Math.round((1 - p.price / p.oldPrice) * 100);
    var imgHtml = p.image ? '<img src="' + p.image + '" style="width:100%; height:100%; object-fit:cover; border-radius:8px;">' : p.emoji;
